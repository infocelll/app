...
function buildTerm(cliente,equip,motivo,osCode){
  ...
  var safeCliente=escH(cliente||'______________');
  var safeEquip=escH(equip||'______________');
  var safeMotivo=escH(motivo||'');
  var safeOsCode=escH(osCode||'');
  return '<h4>TERMO DE RETIRADA SEM APRESENTAÇÃO DA ORDEM DE SERVIÇO</h4>'+
  '<p>Eu, <strong>'+safeCliente+'</strong>'+(motivo?', motivando esta solicitação por <strong>'+safeMotivo+'</strong>':'')+', venho por meio deste solicitar a <strong>RETIRADA</strong> do equipamento <strong>'+safeEquip+'</strong>'+(osCode&&osCode!=='______________'?' vinculado à Ordem de Serviço <strong>'+safeOsCode+'</strong>':'')+' da <strong>InfoCelll</strong>, em <strong>'+dataTxt+'</strong>.</p>'+
