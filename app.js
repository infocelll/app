...
(function(){
  ...
  function toolsByCat(c){return D.filter(function(t){return t.cat===c;});}
  ...
  function runTool(id,autoRun){
    ...
    S.addHistory({tool:t.nome,icon:t.icon,prompt:promptText,preview:promptText.slice(0,90)+'…',date:new Date().toISOString()});
  ...
  function copyOut(){
    ...
    var out=$('ciaOut');
    if(out&&out.classList.contains('show'))txt=out.textContent;
    copyText(txt||'','Prompt copiado!');
  }
  ...
  function savePromptFromOut(){
    ...
    var prompts=S.getPrompts();
    prompts.push({id:'p'+Date.now(),nome:('Prompt '+prompts.length+1),texto:txt,cat:'Geral',criado:new Date().toISOString()});
  ...
  function savePromptEditor(id){
    ...
    if(id){
      ...
      prompts.push({id:'p'+Date.now(),nome:nome,texto:texto,cat:cat,criado:new Date().toISOString()});
    }
    S.savePrompts(prompts);
    closeModal();
    renderPrompts();
  ...
  function duplicatePrompt(id){
    ...
    if(!p)return;
    var prompts=S.getPrompts();
    prompts.push({id:'p'+Date.now(),nome:p.nome+' (cópia)',texto:p.texto,cat:p.cat,criado:new Date().toISOString()});
    S.savePrompts(prompts);
    renderPrompts();
  ...
  function renderHistory(){
    ...
    html+='<div class="cia-list">'+(hist.length?hist.map(function(h,i){
      ...
      return '<div class="cia-list-item"><div class="cia-li-ico">'+(h.icon||'🤖')+'</div><div class="cia-li-main"><b>'+esc(h.tool||'Prompt')+'</b><small>'+esc(ds)+' · '+esc(h.preview||'')+'</small></div>'+
        '<div class="cia-li-actions"><button class="cia-btn cia-sm" onclick="CentralAI.copyText(this.dataset.t)" data-t="'+esc(h.prompt||'')+'">📋</button></div></div>';
  ...
  function renderConfig(){
    ...
    var provOpts=Object.keys(A.PROV).map(function(k){
      var p=A.PROV[k];
      return '<label class="cia-opt'+(c.provider===k?' active':'')+'" onclick="CentralAI.selectProvider(\''+k+'\')"><input type="radio" name="ciaProv" value="'+k+'"'+(c.provider===k?' checked':'')+'><div><b>'+p.name+'</b><small>Modelo padrão: '+p.defModel+'</small></div></label>';
    }).join('');
